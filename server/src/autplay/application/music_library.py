...
class MusicLibraryService:
    ...
    def prepare(self, principal: Principal, ref_id: UUID) -> UUID:
        with self.sessions.begin() as session:
            ...
            ref = self._owned_ref(session, principal, ref_id)
            ...
            if predecessor is None:
                ...
                predecessor = _predecessor(ref, principal)
                session.add(predecessor)
                session.flush()
            if (
                predecessor.owner_user_id != principal.user_id
                or predecessor.user_track_ref_id != ref_id
            ):
            ...
            values.update(
                ...
                idempotency_scope=f"music-prepare:{principal.user_id}",
                ...
                decided_at=max(
                    datetime.now(UTC), predecessor.decided_at + timedelta(microseconds=1)
                ),
            )
            ...
            evidence = tuple(
                MatchCandidateEvidenceRow(
                    **{
                        ...
                        if c.name
                        not in {"match_candidate_evidence_id", "decision_id", "created_at"}
                    }
                )
            ...
            session.flush()
            CatalogArtistSyncPublisher().publish(session, principal.user_id, ref_ids=(ref_id,))
    ...
    def publish(self, principal: Principal, ref_id: UUID, upload_id: UUID) -> UUID:
        with self.sessions.begin() as session:
            ...
            if (
                upload is None
                or upload.user_id != principal.user_id
                or upload.device_id != principal.device_id
            ):
        ...
        with self.sessions.begin() as session:
            _acquire_sync_owner_publish_lock(session, principal.user_id)
            self._owned_ref(session, principal, ref_id)
            ...
            session.execute(
                ...
                .values(
                    event_id=uuid5(
                        NAMESPACE_URL,
                        f"music-publish:{principal.user_id}:{ref_id}:{upload_id}:{entry.row_version}",
                    ),
    ...
    @staticmethod
    def _owned_ref(session: Session, principal: Principal, ref_id: UUID) -> UserTrackRefRow:
        ref = session.scalar(
...
def _predecessor(ref: UserTrackRefRow, principal: Principal) -> MatchDecisionRow:
