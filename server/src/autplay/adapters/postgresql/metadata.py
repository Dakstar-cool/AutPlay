"""Complete PostgreSQL v1 metadata registry and stable inventory constants."""

from __future__ import annotations

from sqlalchemy import MetaData

from .base import Base
from .models import MAPPED_ROWS

metadata: MetaData = Base.metadata

EXPECTED_SCHEMAS: frozenset[str] = frozenset(
    [
        "account",
        "audit",
        "catalog",
        "discovery",
        "identity",
        "importing",
        "jobs",
        "library",
        "ml",
        "playlist",
        "social",
        "sync",
        "vault",
        "wave",
    ]
)
EXPECTED_TABLE_KEYS: tuple[str, ...] = (
    "ml.training_checkpoint",
    "ml.training_execution",
    "ml.training_run",
    "ml.training_participant",
    "ml.training_publication_revocation",
    "ml.training_cleanup_claim",
    "account.training_consent",
    "account.training_consent_operation",
    "account.account_deletion_hold",
    "account.account_deletion_request",
    "account.account_purge_receipt",
    "account.account_recovery_credential",
    "account.account_recovery_operation",
    "account.internal_io_policy",
    "account.resource_quota_policy",
    "account.account_quota_override",
    "account.resource_grant_cursor",
    "account.resource_admission",
    "account.resource_io_permit",
    "account.resource_io_execution",
    "account.quota_operation_receipt",
    "account.self_device_pairing",
    "account.self_pairing_command",
    "account.self_pairing_rate",
    "discovery.internet_search",
    "discovery.internet_acquisition",
    "library.metadata_execution",
    "library.metadata_provider_gate",
    "library.metadata_artwork",
    "library.track_metadata",
    "library.track_metadata_revision",
    "account.user_account",
    "account.device",
    "account.user_session",
    "account.server_instance",
    "account.enrollment_invitation",
    "account.enrollment_exchange_receipt",
    "account.session_rotation_receipt",
    "account.profile_lifecycle_command",
    "account.device_admission",
    "account.device_admission_nonce",
    "account.device_admission_exchange_receipt",
    "account.device_admission_rate_window",
    "account.device_admission_web_operation_receipt",
    "account.device_key_block",
    "account.trusted_device_key",
    "account.trusted_device_reenrollment_challenge",
    "account.web_session_invitation",
    "account.web_login_challenge",
    "account.web_session",
    "account.web_session_rotation_evidence",
    "account.web_terminal_receipt",
    "account.web_login_rate_window",
    "account.web_passkey",
    "account.web_passkey_ceremony",
    "account.web_passkey_revocation",
    "account.account_invitation",
    "account.account_registration_receipt",
    "account.account_provisioning_link",
    "account.account_provisioning_operation_receipt",
    "account.account_provisioning_rate_window",
    "discovery.bulk_operation",
    "discovery.source_authorization",
    "discovery.artist_policy",
    "discovery.artist_policy_revision",
    "discovery.candidate",
    "discovery.bulk_operation_item",
    "discovery.run",
    "discovery.run_page",
    "discovery.run_candidate",
    "discovery.acquisition_attempt",
    "discovery.candidate_action_receipt",
    "catalog.artist",
    "catalog.artist_credit",
    "catalog.artist_credit_name",
    "catalog.work",
    "catalog.recording",
    "catalog.release_group",
    "catalog.release",
    "catalog.medium",
    "catalog.release_track",
    "audit.catalog_change_set",
    "audit.catalog_change_item",
    "audit.audit_event",
    "identity.source_provider",
    "identity.recording_identifier",
    "identity.external_reference",
    "identity.source_observation",
    "identity.matcher_release",
    "identity.calibrator_release",
    "identity.threshold_set",
    "identity.match_policy_activation",
    "identity.recording_redirect",
    "sync.device_event_inbox",
    "sync.sync_event",
    "sync.device_sync_cursor",
    "sync.tombstone",
    "sync.idempotency_record",
    "sync.bootstrap_session",
    "sync.bootstrap_snapshot_item",
    "library.user_interaction_event",
    "jobs.job",
    "jobs.job_attempt",
    "jobs.job_dependency",
    "library.user_track_ref",
    "library.user_track_ref_external_reference",
    "library.library_entry",
    "library.user_track_preference",
    "playlist.playlist",
    "playlist.playlist_entry",
    "playlist.smart_playlist_rule",
    "vault.vault_object",
    "vault.vault_replica",
    "vault.audio_variant",
    "vault.audio_fingerprint",
    "vault.recording_canonical_variant",
    "vault.acquisition_record",
    "vault.provider_staging",
    "vault.orphan_object_claim",
    "vault.upload_cleanup_claim",
    "vault.ingest_execution",
    "vault.ingest_cleanup_claim",
    "vault.ingest_cleanup_execution",
    "vault.provider_maintenance",
    "vault.upload_session",
    "vault.upload_chunk",
    "importing.import_job",
    "importing.import_entry",
    "importing.web_import_operation_receipt",
    "identity.match_decision",
    "identity.match_candidate_evidence",
    "ml.embedding_model",
    "ml.artifact",
    "ml.artifact_migration_issue",
    "ml.artifact_license_decision",
    "ml.artifact_license_current",
    "ml.face_artifact_release",
    "ml.face_execution_profile",
    "ml.face_semantic_interpreter",
    "ml.face_qualification_set",
    "ml.face_qualification_approval",
    "ml.face_timeline_activation",
    "ml.face_activation_current",
    "ml.face_analysis_policy_history",
    "ml.face_analysis_policy_current",
    "ml.sona_artifact_release",
    "ml.control_step_up_credential",
    "ml.control_step_up_challenge",
    "ml.control_step_up_receipt",
    "ml.gpu_device_authority",
    "ml.gpu_reservation_current",
    "ml.gpu_admission_receipt",
    "ml.sona_capture_bundle",
    "ml.sona_capture_lineage_cursor",
    "ml.sona_capture_target_dispatch",
    "ml.sona_shadow_work",
    "ml.sona_shadow_attempt",
    "ml.sona_shadow_evidence",
    "ml.recording_embedding",
    "ml.embedding_benchmark_report",
    "ml.embedding_model_activation",
    "ml.enrichment_job",
    "ml.recording_tag_set",
    "ml.recommendation_pipeline_version",
    "ml.recommendation_input_snapshot",
    "ml.recommendation_temporal_event",
    "ml.recommendation_adaptive_profile",
    "ml.recommendation_temporal_snapshot",
    "ml.recommendation_request",
    "ml.recommendation_item",
    "ml.taste_cluster",
    "ml.taste_cluster_member",
    "ml.offline_recommendation_pack",
    "library.listening_event",
    "wave.room",
    "wave.member",
    "wave.invitation",
    "wave.queue_entry",
    "wave.command",
    "wave.preflight",
    "wave.timing_report",
    "social.friend_request",
    "social.friendship",
    "social.user_block",
    "social.presence_settings",
    "social.profile_statistics_settings",
    "social.presence_heartbeat",
    "social.friend_room_invitation",
    "social.operation_receipt",
    "social.rate_window",
    "social.guest_invitation",
    "social.guest_session",
    "social.guest_operation_receipt",
    "social.guest_preflight",
    "social.guest_timing_report",
    "social.guest_rate_window",
)
EXPECTED_TABLE_COLUMN_COUNTS: dict[str, int] = {
    "ml.training_run": 16,
    "ml.training_participant": 4,
    "ml.training_publication_revocation": 4,
    "ml.training_cleanup_claim": 4,
    "ml.training_checkpoint": 3,
    "ml.training_execution": 36,
    "account.training_consent": 5,
    "account.training_consent_operation": 7,
    "account.account_deletion_hold": 3,
    "account.account_deletion_request": 18,
    "account.account_purge_receipt": 3,
    "account.account_recovery_credential": 8,
    "account.account_recovery_operation": 18,
    "account.internal_io_policy": 14,
    "account.resource_quota_policy": 12,
    "account.account_quota_override": 6,
    "account.resource_grant_cursor": 3,
    "account.resource_admission": 34,
    "account.resource_io_permit": 8,
    "account.resource_io_execution": 20,
    "account.quota_operation_receipt": 9,
    "discovery.internet_search": 7,
    "discovery.internet_acquisition": 17,
    "library.metadata_execution": 21,
    "library.metadata_provider_gate": 4,
    "library.metadata_artwork": 3,
    "library.track_metadata": 10,
    "library.track_metadata_revision": 6,
    "account.user_account": 10,
    "account.self_device_pairing": 21,
    "account.self_pairing_command": 8,
    "account.self_pairing_rate": 3,
    "account.device": 13,
    "account.user_session": 11,
    "account.server_instance": 10,
    "account.enrollment_invitation": 9,
    "account.enrollment_exchange_receipt": 9,
    "account.session_rotation_receipt": 7,
    "account.profile_lifecycle_command": 12,
    "account.device_admission": 29,
    "account.device_admission_nonce": 4,
    "account.device_admission_exchange_receipt": 9,
    "account.device_admission_rate_window": 5,
    "account.device_admission_web_operation_receipt": 10,
    "account.device_key_block": 5,
    "account.trusted_device_key": 8,
    "account.trusted_device_reenrollment_challenge": 9,
    "account.web_session_invitation": 9,
    "account.web_login_challenge": 6,
    "account.web_session": 14,
    "account.web_session_rotation_evidence": 4,
    "account.web_terminal_receipt": 17,
    "account.web_login_rate_window": 4,
    "account.web_passkey": 13,
    "account.web_passkey_ceremony": 16,
    "account.web_passkey_revocation": 5,
    "account.account_invitation": 8,
    "account.account_registration_receipt": 11,
    "account.account_provisioning_link": 4,
    "account.account_provisioning_operation_receipt": 8,
    "account.account_provisioning_rate_window": 5,
    "discovery.bulk_operation": 18,
    "discovery.source_authorization": 18,
    "discovery.artist_policy": 14,
    "discovery.artist_policy_revision": 13,
    "discovery.candidate": 34,
    "discovery.bulk_operation_item": 4,
    "discovery.run": 24,
    "discovery.run_page": 8,
    "discovery.run_candidate": 4,
    "discovery.acquisition_attempt": 15,
    "discovery.candidate_action_receipt": 9,
    "catalog.artist": 12,
    "catalog.artist_credit": 7,
    "catalog.artist_credit_name": 6,
    "catalog.work": 8,
    "catalog.recording": 16,
    "catalog.release_group": 12,
    "catalog.release": 15,
    "catalog.medium": 10,
    "catalog.release_track": 13,
    "audit.catalog_change_set": 10,
    "audit.catalog_change_item": 8,
    "audit.audit_event": 11,
    "identity.source_provider": 11,
    "identity.recording_identifier": 8,
    "identity.external_reference": 15,
    "identity.source_observation": 8,
    "identity.matcher_release": 7,
    "identity.calibrator_release": 6,
    "identity.threshold_set": 12,
    "identity.match_policy_activation": 12,
    "identity.recording_redirect": 5,
    "sync.device_event_inbox": 18,
    "sync.sync_event": 12,
    "sync.device_sync_cursor": 7,
    "sync.bootstrap_session": 8,
    "sync.bootstrap_snapshot_item": 6,
    "library.user_interaction_event": 11,
    "sync.tombstone": 7,
    "sync.idempotency_record": 8,
    "jobs.job": 28,
    "jobs.job_attempt": 9,
    "jobs.job_dependency": 3,
    "library.user_track_ref": 15,
    "library.user_track_ref_external_reference": 4,
    "library.library_entry": 10,
    "library.user_track_preference": 8,
    "playlist.playlist": 10,
    "playlist.playlist_entry": 11,
    "playlist.smart_playlist_rule": 5,
    "vault.vault_object": 11,
    "vault.vault_replica": 9,
    "vault.audio_variant": 17,
    "vault.audio_fingerprint": 12,
    "vault.recording_canonical_variant": 5,
    "vault.acquisition_record": 9,
    "vault.provider_staging": 34,
    "vault.orphan_object_claim": 5,
    "vault.upload_cleanup_claim": 6,
    "vault.ingest_execution": 18,
    "vault.ingest_cleanup_claim": 11,
    "vault.ingest_cleanup_execution": 14,
    "vault.provider_maintenance": 18,
    "vault.upload_session": 29,
    "vault.upload_chunk": 6,
    "importing.import_job": 13,
    "importing.import_entry": 16,
    "importing.web_import_operation_receipt": 5,
    "identity.match_decision": 48,
    "identity.match_candidate_evidence": 15,
    "ml.embedding_model": 29,
    "ml.artifact": 8,
    "ml.artifact_migration_issue": 4,
    "ml.artifact_license_decision": 17,
    "ml.artifact_license_current": 5,
    "ml.face_artifact_release": 7,
    "ml.face_execution_profile": 6,
    "ml.face_semantic_interpreter": 15,
    "ml.face_qualification_set": 15,
    "ml.face_qualification_approval": 18,
    "ml.face_timeline_activation": 15,
    "ml.face_activation_current": 4,
    "ml.face_analysis_policy_history": 15,
    "ml.face_analysis_policy_current": 9,
    "ml.sona_artifact_release": 7,
    "ml.control_step_up_credential": 11,
    "ml.control_step_up_challenge": 17,
    "ml.control_step_up_receipt": 7,
    "ml.gpu_device_authority": 7,
    "ml.gpu_reservation_current": 15,
    "ml.gpu_admission_receipt": 17,
    "ml.sona_capture_bundle": 17,
    "ml.sona_capture_lineage_cursor": 8,
    "ml.sona_capture_target_dispatch": 13,
    "ml.sona_shadow_work": 18,
    "ml.sona_shadow_attempt": 12,
    "ml.sona_shadow_evidence": 6,
    "ml.recording_embedding": 13,
    "ml.embedding_benchmark_report": 10,
    "ml.embedding_model_activation": 10,
    "ml.enrichment_job": 9,
    "ml.recording_tag_set": 12,
    "ml.recommendation_pipeline_version": 9,
    "ml.recommendation_input_snapshot": 10,
    "ml.recommendation_temporal_event": 28,
    "ml.recommendation_adaptive_profile": 12,
    "ml.recommendation_temporal_snapshot": 21,
    "ml.recommendation_request": 39,
    "ml.recommendation_item": 10,
    "ml.taste_cluster": 8,
    "ml.taste_cluster_member": 4,
    "ml.offline_recommendation_pack": 15,
    "library.listening_event": 15,
    "wave.room": 16,
    "wave.member": 8,
    "wave.invitation": 3,
    "wave.queue_entry": 6,
    "wave.command": 12,
    "wave.preflight": 10,
    "wave.timing_report": 9,
    "social.friend_request": 7,
    "social.friendship": 3,
    "social.user_block": 4,
    "social.presence_settings": 6,
    "social.profile_statistics_settings": 4,
    "social.presence_heartbeat": 7,
    "social.friend_room_invitation": 14,
    "social.operation_receipt": 11,
    "social.rate_window": 5,
    "social.guest_invitation": 17,
    "social.guest_session": 16,
    "social.guest_operation_receipt": 15,
    "social.guest_preflight": 9,
    "social.guest_timing_report": 9,
    "social.guest_rate_window": 5,
}
EXPECTED_EXPLICIT_INDEX_NAMES: frozenset[str] = frozenset(
    {
        "ix_training_participant_owner",
        "uq_training_execution_run_open",
        "ix_deletion_request_deadline",
        "ix_deletion_request_pending_user",
        "ix_recovery_operation_expiry",
        "ix_recovery_operation_user",
        "ix_provider_staging_target",
        "ix_provider_staging_cleanup",
        "ix_provider_staging_scratch_pending",
        "uq_orphan_object_claim_active",
        "ix_upload_cleanup_pending",
        "uq_metadata_execution_ref",
        "uq_ingest_execution_upload",
        "uq_ingest_execution_staging",
        "ix_ingest_cleanup_pending",
        "uq_ingest_cleanup_execution_claim",
        "uq_provider_maintenance_active",
        "ix_admission_account_state",
        "ix_admission_lease_expiry",
        "ix_admission_waiting",
        "ix_io_permit_expiry",
        "ix_io_execution_unclosed",
        "ix_io_execution_owner",
        "ix_io_execution_writer",
        "ix_io_permit_operation",
        "ix_quota_receipt_created",
        "internet_search_owner_time",
        "ix_device_user_active",
        "ix_user_session_user_active",
        "ix_enrollment_invitation_user_active",
        "ix_enrollment_exchange_receipt_expiry",
        "ix_session_rotation_receipt_expiry",
        "ix_device_admission_cleanup",
        "ix_device_admission_poll_expiry",
        "uq_device_admission_locator",
        "uq_device_admission_pending_key",
        "ix_device_key_block_active",
        "ix_trusted_reenrollment_challenge_expiry",
        "ix_device_admission_receipt_expiry",
        "ix_device_admission_web_operation_receipt_expiry",
        "ix_device_admission_rate_window_expiry",
        "ix_web_invitation_user_active",
        "ix_web_invitation_expiry",
        "ix_web_login_challenge_expiry",
        "ix_web_session_user_active",
        "ix_web_session_expiry",
        "ix_web_session_rotation_evidence_expiry",
        "ix_web_terminal_receipt_expiry",
        "ix_web_login_rate_window_expiry",
        "ix_web_passkey_user",
        "ix_self_pairing_user_expiry",
        "ix_self_pairing_expiry",
        "ix_self_pairing_command_ceremony",
        "ix_self_pairing_rate_expiry",
        "ix_web_passkey_ceremony_expiry",
        "ix_web_passkey_ceremony_user",
        "ix_web_session_passkey",
        "ix_account_invitation_expiry",
        "ix_account_registration_receipt_expiry",
        "ix_account_provisioning_rate_expiry",
        "ix_bulk_operation_owner_time",
        "ix_discovery_candidate_owner_state",
        "ix_source_authorization_owner_expiry",
        "uq_source_authorization_current_scope",
        "ix_artist_policy_due",
        "ix_discovery_run_policy_slot",
        "ix_acquisition_attempt_candidate",
        "ix_candidate_action_receipt_candidate",
        "uq_acquisition_attempt_active_candidate",
        "ix_bulk_operation_item_candidate",
        "ix_artist_normalized_name_trgm",
        "ix_artist_credit_normalized_name_trgm",
        "ix_artist_credit_name_artist",
        "ix_recording_artist_credit",
        "ix_release_artist_credit_active",
        "ix_recording_normalized_title_trgm",
        "ix_release_group_title_trgm",
        "ix_release_release_group",
        "ix_release_barcode",
        "ix_release_track_recording",
        "ix_audit_event_occurred_at",
        "ix_audit_event_target",
        "ix_recording_identifier_lookup",
        "ix_external_reference_recording",
        "ix_source_observation_reference_time",
        "ix_threshold_set_scope",
        "ix_match_policy_activation_threshold_time",
        "ix_device_event_inbox_pending",
        "ix_sync_event_user_sequence",
        "ix_device_sync_cursor_user",
        "ix_tombstone_retention",
        "ix_idempotency_record_expiry",
        "ix_bootstrap_session_expiry",
        "uq_job_idempotency",
        "ix_job_claim",
        "ix_job_expired_lease",
        "uq_user_track_ref_active_recording",
        "ix_user_track_ref_recording_user_active",
        "ix_user_track_ref_user_status",
        "ix_user_track_ref_external_reverse",
        "uq_library_entry_active",
        "ix_library_entry_page",
        "ix_playlist_owner_active",
        "uq_playlist_entry_active_position",
        "ix_playlist_entry_order",
        "ix_vault_object_status",
        "ix_vault_replica_object_status",
        "ix_audio_variant_recording_valid",
        "ix_audio_fingerprint_candidate",
        "ix_acquisition_record_variant",
        "ix_upload_session_owner_state_time",
        "ix_upload_session_state_expiry",
        "ix_upload_session_computed_sha256",
        "ix_upload_session_job",
        "ix_import_job_user",
        "ix_import_entry_job_status",
        "ix_match_decision_query_time",
        "ix_match_decision_candidate_time",
        "ix_match_decision_matcher_time",
        "ix_match_candidate_evidence_recording",
        "uq_embedding_model_single_active_task",
        "ix_recording_embedding_model_recording",
        "ix_embedding_benchmark_model_time",
        "ix_embedding_activation_task_time",
        "ix_enrichment_job_model_recording",
        "ix_recording_tag_set_model_recording",
        "ix_control_credential_actor",
        "ix_control_step_up_expiry",
        "ix_control_step_up_actor",
        "ix_gpu_current_lease",
        "ix_gpu_receipt_device_time",
        "ix_sona_capture_owner_expiry",
        "ix_sona_cursor_active_owner_expiry",
        "ix_sona_dispatch_owner_state",
        "ix_sona_work_claim_owner",
        "ix_sona_attempt_owner_work",
        "ix_sona_evidence_owner_request",
        "ix_face_interpreter_encoder",
        "ix_face_qualification_state_expiry",
        "ix_face_approval_state_expiry",
        "uq_face_policy_actor_operation",
        "ix_face_policy_owner_created",
        "ix_face_policy_current_effective",
        "ix_recommendation_snapshot_user_retention",
        "ix_recommendation_temporal_event_owner_watermark",
        "ix_recommendation_temporal_event_retention",
        "ix_recommendation_adaptive_profile_owner_cutoff",
        "ix_recommendation_temporal_snapshot_owner_retention",
        "ix_recommendation_request_user_time",
        "ix_recommendation_item_recording",
        "ix_taste_cluster_user_active",
        "ix_offline_pack_user_device",
        "ix_listening_event_user_time",
        "ix_listening_event_recording_time",
        "ix_wave_room_expiry",
        "ix_wave_member_presence",
        "ix_wave_preflight_room_recording",
        "uq_social_pending_friend_request",
        "ix_social_friend_request_target",
        "ix_social_user_block_active",
        "ix_social_presence_fresh",
        "ix_social_room_invitation_target",
        "uq_social_pending_room_target",
        "ix_social_operation_receipt_expiry",
        "ix_social_rate_window_expiry",
        "ix_social_guest_invitation_room_state",
        "ix_social_guest_invitation_expiry",
        "ix_social_guest_session_room_state",
        "ix_social_guest_session_expiry",
        "ix_social_guest_operation_expiry",
        "ix_social_guest_preflight_expiry",
        "ix_social_guest_timing_reported",
        "ix_social_guest_rate_expiry",
    }
)
EXPECTED_COLUMN_COUNT = 2216

__all__ = (
    "EXPECTED_COLUMN_COUNT",
    "EXPECTED_EXPLICIT_INDEX_NAMES",
    "EXPECTED_SCHEMAS",
    "EXPECTED_TABLE_COLUMN_COUNTS",
    "EXPECTED_TABLE_KEYS",
    "MAPPED_ROWS",
    "metadata",
)
