"""Deployment-pinned reviewer trust for Sona quality admission."""
...
def load_deployment_sona_reviewer_trust_anchor(
    ...
    """Load the reviewer only from deployment configuration, never request payloads."""
    ...
    if not 1 <= size <= SONA_MAX_REVIEWER_SPKI_BYTES or len(reviewer_spki) != size:
        raise RuntimeError("Sona quality reviewer SPKI size is invalid")
    try:
        return SonaReviewerTrustAnchor(thumbprint or "", reviewer_spki)
