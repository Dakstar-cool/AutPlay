...
def upgrade() -> None:
    ...
    CREATE TABLE discovery.internet_acquisition (
        ...
        state text NOT NULL DEFAULT 'QUEUED'
          CHECK(state IN ('QUEUED','DOWNLOADING','UPLOADING','PROCESSING','READY','FAILED')),
    ...
    BEGIN
      IF TG_TABLE_NAME='internet_search' OR
         ROW(NEW.acquisition_id,NEW.user_id,NEW.device_id,NEW.search_id,
             NEW.candidate_id,NEW.selected_snapshot,NEW.job_id)
         IS DISTINCT FROM ROW(OLD.acquisition_id,OLD.user_id,OLD.device_id,OLD.search_id,
             OLD.candidate_id,OLD.selected_snapshot,OLD.job_id)
