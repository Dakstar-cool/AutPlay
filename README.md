...
Начните с [доказательств тестирования RC1](docs/release/TEST_EVIDENCE.md), [обзора безопасности](docs/release/SECURITY_REVIEW.md) и [отчёта о производительности](docs/release/PERFORMANCE_REPORT.md). Каждое утверждение о приёмке связано с исполняемым доказательством в [матрице приёмки MVP](docs/build-pack/MVP_ACCEPTANCE_MATRIX.md).
...
`bootstrap` отдельно синхронизирует зафиксированные окружения инструментов репозитория и CPU‑сервера, разрешает Gradle Wrapper и проверяет конфигурацию Compose. `check` повторяет фиксированную настройку, проверяет контракты, запускает проверки harness и качества сервера, аудирует граф CPU‑зависимостей, выполняет Android‑проверки на хосте, если не указан режим только сервера, и создаёт одноразовый проект PostgreSQL с уникальным именем. Одноразовая база публикуется только на случайном loopback‑порту и после выполнения удаляется вместе со своей сетью и томом.
...
Независимая от хостинга матрица описана в [плане CI](docs/implementation/CI_PLAN.md), а доказательства локального RC — в [передаче результатов P14](docs/implementation/HANDOFF_P14.md).
...
Допустимые селекторы: `auto`, `uuid:<GPU UUID>`, `pci:<PCI bus ID>` и `index:<n>`. Для устойчивого ручного выбора используйте UUID или PCI. В RC1 ни одна GPU‑модель не упакована и не активирована; авторитетным остаётся детерминированный базовый CPU‑алгоритм рекомендаций. Подробности — в [ADR-025](docs/adr/ADR-025-p12-isolated-gpu-enrichment-and-model-rollout.md) и [передаче результатов P12](docs/implementation/HANDOFF_P12.md).

</details>

<details>
<summary><strong>Использовать локальную среду разработки Codex</strong></summary>

Среда Codex относится к инструментам разработки и не является частью рабочего контура AutPlay. Её корневой lock‑файл намеренно отделён от `server/uv.lock`.

```powershell
uv run --frozen autplay-codex status
uv run --frozen autplay-codex task "Опиши одну ограниченную задачу репозитория" --dry-run
...
Команды с правом записи отказываются работать с защищёнными ветками, грязным рабочим деревом, разрушительными запросами и неограниченным выполнением. Источником истины о фазах продукта остаётся [план реализации](docs/implementation/PLAN.md); состояние среды в `.autplay-codex/` — локальное рабочее состояние, а не доказательство продукта.
...
Прежде чем считать репозиторий чем-то большим, чем локальный кандидат, прочитайте полные [примечания к релизу RC1](docs/release/RELEASE_NOTES_RC1.md), [реестр рисков](docs/implementation/RISK_REGISTER.md) и [инструкцию по резервному копированию и восстановлению](docs/operations/BACKUP_RESTORE.md).
